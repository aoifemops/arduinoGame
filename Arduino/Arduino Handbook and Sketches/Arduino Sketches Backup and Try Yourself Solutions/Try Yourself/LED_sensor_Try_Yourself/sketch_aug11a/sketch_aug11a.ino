...
{      
  ...
  {
    lightLevelR+=1;                   // the value after the += gives how quickly the light goes on 
                                      // this one increases one at a time
    if(lightLevelR>255){
      lightLevelR=0;
      switcher=1;                     // once it's on fully change the switcher to move to yellow LED
    } 
  } else                              // gradually switch on the red LED
  {
    lightLevelY+=2;                   // the value after the += gives how quickly the light goes on 
                                      // this one increases two at a time so should be fully on in half
                                      // the time of the red LED              
    if(lightLevelY>255){
      lightLevelY=0;
      switcher=0;
    }
